...
% Instead of SumOfSquaredDiff, compute the Normalized Cross-Correlation (NCC)
NormalizedCrossCorrelation = zeros(n, m);
for i = 1:n
    for j = 1:m
        patchA_norm = (patchA(:,:,i) - mA(i)) / sA(i);
        patchB_norm = (patchB(:,:,j) - mB(j)) / sB(j);
        NormalizedCrossCorrelation(i, j) = sum(sum(patchA_norm .* patchB_norm));
    end
end
...
[sorted_ssd,id_ssd]=sort(pairs(:,3),1,'descend');
