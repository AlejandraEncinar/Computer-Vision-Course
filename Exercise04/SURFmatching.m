...
% Calculate the NNDR for each feature in f1
nn_ratio = pairs(:,3);
for k=1:size(pairs,1)
    sd=sort(distmat(pairs(k,1),:),2,'ascend');
    % Calculate NNDR using Equation (4.18)
    nn_ratio(k) = sd(1) / sd(2);
end

% Sort the features based on NNDR in ascending order
[sorted_nn_ratio, sorted_indices] = sort(nn_ratio, 1,'ascend');
...
% Determine how many of the top 5 matches are correct correspondences
correct_matches = 0;
for k = 1:Nvis
    idx = idlist(k);
    pid1 = pairs(idx, 1);
    ...
    % Calculate the transformed location in the second image
    p1to2 = H1to2p * [vpts1(pid1).Location, 1]';
    p1to2 = p1to2(1:2) / p1to2(3);
    
    % Calculate the Euclidean distance between the transformed location and
    % the location in the second image
    distance = sqrt(sum((p1to2 - vpts2(pid2).Location').^2,2))
    
    % If the distance is within a threshold (e.g., 2 pixels), consider it a
