...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % Compute ∂E/∂z^(2)
  dE_dz2 = (class_prob-data.targets)';

  % Compute ∂E/∂y^(1)
  dE_dy1 = (dE_dz2 * model.hid_to_class);

  % Compute ∂E/∂z^(1)
  dE_dz1 = dE_dy1 * diag(hid_output .* (1 - hid_output));

  % Compute ∂E/∂W^(2)
  dE_dW2 = dE_dz2' * hid_output';

  % Compute ∂E/∂W^(1)
  dE_dW1 = dE_dz1' * data.inputs';

  % Assign the computed gradients to the return structure
  ret.input_to_hid = dE_dW1;
  ret.hid_to_class = dE_dW2;
